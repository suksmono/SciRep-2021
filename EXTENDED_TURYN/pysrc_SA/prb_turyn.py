...
def construct_XYZW(v):
    ...
    return(X,Y,Z,W)
#
def generate_XYZW_normal_SA(q,N):
    x=np.zeros([N,1])
    y=np.zeros([N,1])
    z=np.zeros([N,1])
    #w=np.zeros([N,1])
...
# check Nx+Ny+2Nz+2Nw=0 on TT-seq
    ...
    return(SS)
##
def tt_energy_xyzw_sqr(X,Y,Z,W):
#---
# check x^2+y^2+2*z^2+2*w^2=6N-2 on TT-seq
#---
    N=len(X)
    # W1=np.zeros([N,1])
    # for m in range(len(W)):
    ...
    w=sum(W)
    #
#    E= 6*N*(x**2 + y**2 + 2*z**2 + 2*w**2) - (6*N-2)
    E= (x**2 + y**2 + 2*z**2 + 2*w**2) - (6*N-2)
    E=E**2
    #return(E)
    return(sum(E))
##
##
def tt_energy_rev(X,Y,Z,W):
# check Nx+Ny+2Nz+2Nw=0 on TT-seq
    N=len(X)
    W1=np.zeros([N,1])
    for m in range(len(W)):
    ...
    #
    E=0
    for m in range(1,len(Nx)):
        tE= Nx[m]+Ny[m]+2*Nz[m]+2*Nw[m]
        E=E + abs(sum(tE))
    #
    return(E)
#
